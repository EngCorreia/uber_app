...
class AppStateProvider with ChangeNotifier {
  ...
  LatLng _center = LatLng(-8.9066025,13.2221661);
...
//  this stream is for all the driver on the app
  ...
  // Inicialização recomendada (por exemplo no initState ou em uma função init)
  void initializeFCM() async {
    // Solicita permissão no iOS
    await fcm.requestPermission();

    // Obter o token (se necessário)
    String? token = await fcm.getToken();
    print("FCM Token: $token");

    // Escuta quando o app está em primeiro plano
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      await handleOnMessage(message.data);
    });

    // Quando o app é iniciado pela notificação (app fechado)
    ...
    // Quando o app está em segundo plano e a notificação é clicada
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) async {
      await handleOnResume(message.data);
    });
  }
...
// ANCHOR: MAPS & LOCATION METHODS
  ...
  Future<Position> _getUserLocation() async {
    ...
    // Obter localização reversa (endereço)
    ...
    if (prefs.getString(COUNTRY) == null) {
      final country = placemark.first.isoCountryCode?.toLowerCase() ?? "ao";
  ...
  Future sendRequest({LatLng? origin, LatLng? destination}) async {
    ...
    RouteModel route = await _googleMapsServices.getRouteByCoordinates(org, _dest);
    routeModel = route;
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      Visibility(
                        visible: driverModel!.photo.isEmpty,
                        child: Container(
                      ...
                      Visibility(
                        visible: driverModel!.photo.isNotEmpty,
                        child: Container(
                          decoration: BoxDecoration(
          ...
          switch (doc.doc['status']) {
            ...
            case ACCEPTED:
              ...
              driverModel = await _driverService
                  .getDriverById(doc.doc['driverId']);
